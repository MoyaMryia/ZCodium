...
- 唯一所有者：`packages/desktop/electron-builder.config.js` 的 `publish.updaterCacheDirName`。
- 取值来源：`desktopProductIdentity.appId`（`packages/desktop/scripts/desktop-product-identity.mjs`），
  production 为 `dev.zcodium.app`，preview 为 `dev.zcodium.app.preview`。
- 运行时读取方 `electron-updater` 只消费该字段，不新增第二处配置。

## 验收场景

1. 打包产物的 `app-update.yml` 中 `updaterCacheDirName` 等于该构建的 appId，不再是
   `@zcodedesktop-updater`。
2. production 与 preview 身份的构建得到不同的缓存目录名，两者互不覆盖。
3. 构建配置中不存在 `@zcodedesktop-updater` 字面量。
