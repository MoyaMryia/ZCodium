...
import { recordMcpDiagnostic } from "./desktopMcpTelemetry.js";
import {
  registerDesktopStabilityMonitors,
  registerStabilityMainWindow,
  recordAgentProcessStarted,
  recordAgentProcessReady,
  recordAgentProcessExited,
  recordAgentProcessError,
  recordAgentProcessException,
...
// 内存诊断计数器：localMemoryDiagnostics 每 60s collect
...
ipcMain.on(PlatformChannels.ReportDiagnostic, (_event, input: unknown) => {
  ...
  if (parsed.success) recordDesktopDiagnostic(parsed.data);
});
const localTtftDiagnostics = createLocalTtftDiagnostics({
  logger,
  onRecord: recordDesktopDiagnostic,
});
ipcMain.on(PlatformChannels.ReportLocalTtftBatch, (_event, batch: unknown) =>
  localTtftDiagnostics.record(batch),
);
...
async function prepareAppQuit(reason: string, kind: AppShutdownKind = "normal"): Promise<void> {
  ...
  stopDesktopNetworkTelemetry();
  void shutdownDesktopDiagnostics();
  notifyStabilityAppExit(
    activeAppShutdownKind === "update-install" ? "update_install" : "app_quit",
  );
    ...
    spawnHostProcess: (win, label, initMessage) =>
      spawnHostProcess(
        ...
        },
        {
          ...
          onAgentProcessExited: recordAgentProcessExited,
          onAgentProcessError: recordAgentProcessError,
          onAgentProcessException: recordAgentProcessException,
          onAgentProcessReady: recordAgentProcessReady,
          onAgentProcessSpawned: recordAgentProcessStarted,
          onMcpTelemetry: (message) => recordMcpDiagnostic(message.event),
...
app.whenReady().then(async () => {
  ...
  registerDesktopStabilityMonitors(logger);
