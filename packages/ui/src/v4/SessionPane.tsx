...
export function SessionPane({
  ...
  useSessionOpenDiagnostics({
    ...
    enabled: sessionLeaseReady && newlyCreatedSessionIdRef.current !== sessionId,
    readOnly,
  });
  useEffect(() => {
    if (newlyCreatedSessionIdRef.current !== sessionId) newlyCreatedSessionIdRef.current = null;
  }, [sessionId]);
  useEffect(() => {
    if (state.status !== "error" || !state.lastError) return;
    recordChatError({
      error: { code: "SESSION_SUBSCRIPTION_ERROR", message: state.lastError },
      displayMessage: state.lastError,
    });
  ...
  const dispatchCommand = useCallback(
    async (
      ...
      try {
        ...
        ack = await measureOperation("command", () => sendCommand(envelope));
