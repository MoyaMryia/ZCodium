...
- 日志、trace 和调试输出（包括 debug）不得采集或记录密钥、token、隐私数据、用户路径、模型输入输出和原始错误全文。统一使用安全诊断边界保留数值、固定状态、标准错误码和应用源码栈位置。
- 业务 `traceId` 继续按执行上下文传递；诊断关联使用本次运行随机生成的独立 ID，不输出或持久化 session/task/request 等业务身份，也不得从隐私信息派生诊断标识。
