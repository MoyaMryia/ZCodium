...
function buildInitAgentsPrompt(params: {
  ...
  return [
    ...
    `- Existing hidden instruction candidates: ${join(params.workingDirectory, ".zcodium", "AGENTS.md")} and ${join(params.workingDirectory, ".agents", "AGENTS.md")}`,
    "- File name must be exactly AGENTS.md.",
    "- This command targets the current workspace only. Do not write ~/.zcodium/AGENTS.md.",
    additionalInstructions,
    "",
    "Process:",
    "1. First check whether .zcodium/AGENTS.md or .agents/AGENTS.md exists in the workspace. If either exists, tell the user they already have an instructions file, mention the path found, and stop without creating a new AGENTS.md.",
