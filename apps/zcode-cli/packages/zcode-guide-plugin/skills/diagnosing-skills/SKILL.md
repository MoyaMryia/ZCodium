...
2. User `~/.zcodium/skills`
3. User `~/.agents/skills`
4. Workspace `.zcodium/skills` — from the working directory up to the repository root, every level counted, deeper wins
5. Workspace `.agents/skills`
6. Enabled **plugin** roots, last

Inside one level `.zcodium` is read before `.agents`. Because a skill's identity is its **file path**, same-named skills at different paths are all discovered, yet only the first in that order is loaded — everything after it is shadowed. Names starting with `.` (`.system` excepted) and `node_modules` are never scanned.
...
1. **Never appears, wrong directory.** Not directly under a discovery root, or the file is not spelled `SKILL.md` (case matters on Linux). → Move it to `~/.zcodium/skills/<name>/SKILL.md` or `<repo>/.zcodium/skills/<name>/SKILL.md`.
2. **Never appears, dot-directory.** Something like `~/.zcodium/skills/.foo/` is skipped because names starting with `.` are excluded, `.system` aside. → Drop the leading `.`.
...
5. **A known-good skill vanishes.** Configuration disabled it by absolute path. → In `~/.zcodium/cli/config.json` or the workspace `.zcodium/config.json`, set that path's `enable` to `true` or delete the override.
...
9. **`.agents` copy ignored.** Within a level `.zcodium` is read first, so a same-named `.zcodium` copy takes the slot. → Remove the `.zcodium` duplicate or rename one of them.
...
1. **Is the feature on?** Check the skill feature and `skills.enabled` in `~/.zcodium/cli/config.json` and the workspace file. Either one false means no discovery at all — pitfall 12.
