...
|---|---|---|
| Configuration file | `~/.zcodium/cli/config.json` | `<repo>/.zcodium/config.json` (or `<repo>/zcode.json`) |
| Instruction file | `~/.zcodium/AGENTS.md` | `<repo>/AGENTS.md` |
...
|---|---|---|---|---|
| **Skills** | Directory + `SKILL.md` | `~/.zcodium/skills/`, `~/.agents/skills/` | `<repo>/.zcodium/skills/`, `<repo>/.agents/skills/` | Identity is the file path; the **first same-named skill in discovery order wins**, and user scope precedes workspace |
| **Commands** | A `.md` file | `~/.zcodium/commands/`, `~/.agents/commands/` | `<repo>/.zcodium/commands/`, `<repo>/.agents/commands/` | Deduplicated by normalized name; **first match wins**, the other is ignored |
| **MCP** | A JSON object | `~/.zcodium/cli/config.json` → `mcp.servers` (fallback `~/.agents/mcp.json` → `mcpServers`) | `<repo>/.zcodium/config.json` → `mcp.servers` (fallback `<repo>/.agents/mcp.json` → `mcpServers`) | **User beats workspace** for a same-named server; workspace servers are **trusted and auto-connected** just like user ones |
| **Hooks** | `hooks.json` or a config object | `~/.zcodium/cli/config.json` → `hooks` | `<repo>/.zcodium/config.json` → `hooks` | Config-file hooks need `hooks.enabled: true`; plugin hooks are appended after them |
| **Plugins** | Directory + `plugin.json` | Installed from a marketplace; state lives in `~/.zcodium/cli/config.json` | — | One plugin contributes skills, commands, hooks, MCP servers and agents |
| **Instructions** | An `AGENTS.md` file | `~/.zcodium/AGENTS.md` | `<repo>/AGENTS.md` | User defaults load first, workspace loads after, so workspace can narrow or override |

> `.agents/mcp.json` is a **compatibility fallback**. Within one scope the client reads `.zcodium` first and only falls back to `.agents/mcp.json` when that scope defines no MCP servers at all. Note the key shapes differ: `.zcodium` nests under `mcp.servers`, while `.agents/mcp.json` uses a top-level `mcpServers`.
...
- `~/.zcodium/AGENTS.md` — personal defaults for every workspace: preferred language, review style, local conventions.
...
- **Creating and migrating** — onboarding can copy `~/.claude/CLAUDE.md` into `~/.zcodium/AGENTS.md`. The built-in `/init` targets the workspace file, so it creates or updates repository instructions rather than touching the user default.
...
2. User `~/.zcodium/skills` (or `commands`)
3. User `~/.agents/skills`
4. Workspace `.zcodium/skills` — every directory from the working directory up to the repository root counts
5. Workspace `.agents/skills`
6. Enabled **plugin** roots, lowest of all

Within a level `.zcodium` is read before `.agents`, and a deeper working-directory location beats one at the repository root.
...
Managed in **Settings → Plugin Management**, which has **Installed** and **Discover** tabs. The manifest lives at `.zcodium-plugin/plugin.json`; `.claude-plugin/` and `.codex-plugin/` are accepted as compatibility names. Only `name` is required, and it must match `^[a-z0-9][a-z0-9._-]{0,127}$`.
...
Enable/disable state lives under `plugins` in `~/.zcodium/cli/config.json`. A built-in plugin can be switched off but not removed. Marketplaces are added from the **`+`** button on Discover, accepting a GitHub repository, a Git URL, a local directory or a file.

## Picking a location

- Personal and cross-project → user scope (`~/.zcodium/...` or `~/.agents/...`).
- Shared with a team and versioned with the repo → workspace scope.
- Shared with other tools (Claude, Codex, Cursor) → `~/.agents/skills/`. To shadow a same-named skill only inside ZCode, use `.zcodium/skills/`.
- Broad personal instructions → `~/.zcodium/AGENTS.md`; project rules → `<repo>/AGENTS.md`.
