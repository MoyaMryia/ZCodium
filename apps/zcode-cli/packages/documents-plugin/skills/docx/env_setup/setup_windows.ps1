# Environment setup for the docx skill (Windows).
#
# Checks what the skill needs and installs what is missing. Idempotent: on a
# working environment it changes nothing. The dependency list is Python +
# defusedxml; LibreOffice is offered explicitly, never installed silently and
# never substituted with Word/WPS/Pages.
#
...
Write-Host "== docx skill environment (Windows) =="
...
if ($python) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "MISSING  defusedxml"
        $needInstall = $true
    }
}

if (Test-Tool "soffice") {
    Write-Host "ok       soffice (on-demand: .doc->.docx, DOCX->PDF, visual verification)"
}
else {
    Write-Host "MISSING  soffice (on-demand - install only when a task needs conversion)"
    $needInstall = $true
}
...
if ($python) {
    ...
    if ($LASTEXITCODE -ne 0) {
        if ($Yes -or ((Read-Host "install defusedxml? [y/N]") -match '^[yY]')) {
            try {
                & $python -m pip install defusedxml
                Write-Host "installed: defusedxml"
            }
            catch {
                Write-Warning "pip install failed - try: $python -m pip install --user defusedxml"
            }
        }
    }
}
...
if (-not (Test-Tool "soffice")) {
    if (Test-Tool "winget") {
        Install-WithWinget "TheDocumentFoundation.LibreOffice" "LibreOffice (conversion and visual checks)"
    }
    else {
        Write-Warning "winget not found - install LibreOffice from https://libreoffice.org; it is never substituted with Word/WPS/Pages"
    }
}
